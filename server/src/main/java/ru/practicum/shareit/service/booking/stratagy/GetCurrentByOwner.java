...
public class GetCurrentByOwner implements BookingStateStrategy {
    private final BookingRepository bookingRepository;
    private final Sort sort = Sort.by(Sort.Direction.DESC, "start");

    @Override
    public List<Booking> findBookings(Long userId) {
        return bookingRepository.findByItemOwnerIdAndStartIsBeforeAndEndIsAfter(userId, LocalDateTime.now(), LocalDateTime.now(), sort);
    }
}
